          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              ...
              >
                ...
                {link.name}
                <span
                  className={`absolute bottom-0 left-0 h-0.5 bg-green-600 transition-all duration-300 rounded-full ${activeTab === link.id ? "w-full" : "w-0 group-hover:w-full"}`}
                ></span>
              </button>
            ))}
          </div>
                      ...
                      notifications.map((n) => (
                        <div
                          ...
                          onClick={() => {
                            ...
                            onTabChange(
                              n.type === "message" ? "messages" : "orders",
                            );
                          }}
            ...
            >
              {link.icon && (
                <link.icon
                  size={20}
                  className={
                    activeTab === link.id ? "text-green-500" : "text-gray-400"
                  }
                />
              )}
              {link.name}
