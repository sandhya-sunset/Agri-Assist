  ...
  return inboundNodes.map((node) => {
    ...
    if (node.args) {
      ...
      const extractHistory = (tensor) => {
        if (
          tensor.class_name === "__keras_tensor__" &&
          tensor.config &&
          tensor.config.keras_history
        ) {
...
if (
  ...
  modelData.modelTopology.model_config &&
  modelData.modelTopology.model_config.config &&
  modelData.modelTopology.model_config.config.layers
) {
  console.log("Found model config. Processing layers...");

  const layers = modelData.modelTopology.model_config.config.layers;
  const modelName =
    modelData.modelTopology.model_config.config.name || "sequential";
  ...
  // 3. Fix weight names - remove model name prefix if present
  if (modelData.weightsManifest && modelData.weightsManifest.length > 0) {
    ...
    if (manifest.weights) {
      ...
      manifest.weights.forEach((weight) => {
        // Remove model name prefix (e.g., "sequential/") from weight names
        if (weight.name && weight.name.startsWith(modelName + "/")) {
          const originalName = weight.name;
          weight.name = weight.name.substring(modelName.length + 1);
          if (originalName !== weight.name) {
            fixedCount++;
          }
        }
      });
      if (fixedCount > 0) {
        console.log(
          `Fixed ${fixedCount} weight names by removing "${modelName}/" prefix`,
        );
      }
    }
  }
  ...
  console.error(
    "Could not find layers in modelTopology. Check file structure.",
  );
}
