  ...
  try {
    ...
    const customersChange =
      prevCustomers.size > 0
        ? (
            ((uniqueCustomers.size - prevCustomers.size) / prevCustomers.size) *
            100
          ).toFixed(1)
        : 0;
    ...
    const trafficSources = [
      {
        source: "Direct",
        percentage: 45,
        visits: Math.floor(totalOrders * 16),
        color: "blue",
      },
      {
        source: "Search Engines",
        percentage: 35,
        visits: Math.floor(totalOrders * 12.5),
        color: "green",
      },
      {
        source: "Social Media",
        percentage: 20,
        visits: Math.floor(totalOrders * 7),
        color: "purple",
      },
    ];
    ...
    const conversionRate =
      totalProductViews > 0
        ? ((totalOrders / totalProductViews) * 100).toFixed(1)
        : 3.2;
    ...
    const returnRate =
      uniqueCustomers.size > 0
        ? ((repeatCustomers / uniqueCustomers.size) * 100).toFixed(0)
        : 42;
    ...
    const avgRating =
      productsWithRatings.length > 0
        ? (
            productsWithRatings.reduce((sum, p) => sum + p.rating, 0) /
