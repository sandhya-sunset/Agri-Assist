...
exports.initiatePayment = async (req, res) => {
  try {
    ...
    if (!location || !location.lat || !location.lng) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Delivery location is required" });
    }
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    if (order.paymentStatus === "Completed") {
      console.log(
        "Payment already verified for this order. Skipping stock reduction.",
      );
    ...
    for (const item of order.items) {
      ...
      if (product) {
        ...
        await product.save();
        console.log(
          `Reduced stock for product ${product.name}: ${previousStock} -> ${product.stock} (${item.quantity} units)`,
        );
